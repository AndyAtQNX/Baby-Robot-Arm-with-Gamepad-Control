...
IKSolverNode::IKSolverNode() : Node("ik_solver_node")
{
  ...
  this->declare_parameter<std::string>(
      "end_effector_link", "Gripper_Assembly_1");
  ...
  auto cart_min = this->get_parameter("cart_min_limits")
                      .as_double_array();
  auto cart_max = this->get_parameter("cart_max_limits")
                      .as_double_array();
  ...
  else
  {
    RCLCPP_WARN(this->get_logger(),
                "Cartesian limits arrays too short. Using defaults.");
  }

  RCLCPP_INFO(this->get_logger(), "Cartesian limits:");
  RCLCPP_INFO(this->get_logger(),
              "  X: [%.3f, %.3f] m", cart_x_min_, cart_x_max_);
  RCLCPP_INFO(this->get_logger(),
              "  Y: [%.3f, %.3f] m", cart_y_min_, cart_y_max_);
  RCLCPP_INFO(this->get_logger(),
              "  Z: [%.3f, %.3f] m", cart_z_min_, cart_z_max_);

  // --- Load Other Parameters ---

  std::string urdf_filename =
      this->get_parameter("urdf_file").as_string();

  RCLCPP_INFO(this->get_logger(),
              "urdf_file parameter value: '%s'", urdf_filename.c_str());
  ...
  RCLCPP_INFO(this->get_logger(),
              "  URDF file: %s", urdf_file_.c_str());
  RCLCPP_INFO(this->get_logger(),
              "  Base link: %s", base_link_.c_str());
  RCLCPP_INFO(this->get_logger(),
              "  End effector: %s", end_effector_link_.c_str());
  ...
  if (!loadURDF())
  {
    RCLCPP_ERROR(this->get_logger(),
                 "Failed to load URDF! Node will exit.");
    rclcpp::shutdown();
    return;
  }
  ...
  {
    RCLCPP_ERROR(this->get_logger(),
                 "Failed to initialize solvers. Exiting.");
    rclcpp::shutdown();
    return;
  }
  ...
  cartesian_subscription_ =
      this->create_subscription<std_msgs::msg::Float64MultiArray>(
          "/CartesianCmd",
          10,
          std::bind(&IKSolverNode::cartesian_callback,
  ...
  pos_subscription_ =
      this->create_subscription<std_msgs::msg::Float64MultiArray>(
          "/CurrentPositions",
          10,
          std::bind(&IKSolverNode::current_positions_callback,
                    this, std::placeholders::_1));
}
...
std::string IKSolverNode::findURDF(const std::string &filename)
{
  ...
  std::string full_path =
      std::string(config_dir) + "/" + filename;

  RCLCPP_INFO(this->get_logger(),
              "Looking for URDF at: '%s'", full_path.c_str());
  ...
  {
    RCLCPP_ERROR(this->get_logger(),
                 "  URDF not found at path: '%s'",
                 full_path.c_str());
    return "";
  }
}

bool IKSolverNode::loadURDF()
{
  RCLCPP_INFO(this->get_logger(),
              "Loading URDF from: %s", urdf_file_.c_str());
  ...
  if (!kdl_tree.getChain(
          base_link_, end_effector_link_, kdl_chain_))
  {
    RCLCPP_ERROR(this->get_logger(),
...
void IKSolverNode::extractJointNames()
{
  joint_names_.clear();
...
bool IKSolverNode::initSolvers()
{
  // FK solver: joint angles → Cartesian position
  fk_solver_ =
      std::make_unique<KDL::ChainFkSolverPos_recursive>(kdl_chain_);
  ...
  ik_pos_solver_ =
      std::make_unique<KDL::ChainIkSolverPos_LMA>(kdl_chain_, L);
  ...
  RCLCPP_INFO(this->get_logger(),
              "  Joints: %u", num_joints_);
  RCLCPP_INFO(this->get_logger(),
              "  Home cartesian position: [%.3f, %.3f, %.3f]",
              initial_frame.p.x(),
              initial_frame.p.y(),
              initial_frame.p.z());
  return true;
...
bool IKSolverNode::clampCartesianTarget(KDL::Frame &target)
{
  bool clamped = false;
    ...
    const std_msgs::msg::Float64MultiArray::SharedPtr msg)
{
  ...
  if (flag == 0.0)
  {
    ...
    fk_solver_->JntToCart(
        current_joint_positions_, cartesian_target_);
  }
}
    ...
    const std_msgs::msg::Float64MultiArray::SharedPtr msg)
{
  ...
  clampCartesianTarget(cartesian_target_);
...
void IKSolverNode::publishJointCommand(
    const KDL::JntArray &joint_angles)
{
